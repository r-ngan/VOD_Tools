...
class VideoSourceCV2(VideoSource.VideoSource):
    ...
    def proc_frame(self):
        if not self.pause:
            if not self.jog_frame(): # jog one frame
                raise VideoSource.VideoException()
        frame_num = self.frame_num
        timestamp = int(self.get_video_ts()) # ms resolution
        frame = self.frame
        lframe = self.lframe
        return frame_num, timestamp, frame, lframe

VideoSource.instance = VideoSourceCV2()